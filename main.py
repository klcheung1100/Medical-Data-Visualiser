...
df['overweight'] = (df["weight"]/((df["height"]/100)**2)).apply(lambda x: int('1') if x > 25 else int('0'))
...
df['cholesterol'] = df['cholesterol'].apply(lambda x: int('1') if x > 1 else int('0'))
df['gluc'] = df['gluc'].apply(lambda x: int('1') if x > 1 else int('0'))
